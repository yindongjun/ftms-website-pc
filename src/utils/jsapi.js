...
var YK = {};
YK.https = window.location.protocol == "https:" ? "https:" : "http:";
...
function createIFrame(w, h, parentName, vid, partnerId, password, autoplay, events) {
  if (YKP.isPC) {
    ...
    var srcUrl = StaticDomain + '/embed/' + vid + '?client_id=' + partnerId + '&password=' + password +
      '&autoplay=' + autoplay;
...
window.QS = function () {
  ...
  if (result != null) {
    for (var i = 0; i < result.length; i++) {
      ...
      val == "true" ? val = true : (val == "false" ? val == false : isNaN(val) ? val = parseJsonStr(val) : val = +
        val);
...
YoukuPlayerSelect.prototype = {
  ...
  selectHandler: function () {
    ...
    if (this.isPC()) {
      ...
      url = YK.https + "//player.youku.com/unifull/js/unifull.min.js?v=20181018";
    }
